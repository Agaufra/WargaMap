...
import { Search, Car, Bike, Footprints, Clock, X, MapPin, Loader2, Navigation, Copy, ArrowDownUp, CheckCircle, ChevronLeft, ChevronRight, Route } from 'lucide-react';
import axios from 'axios';

const RoutingPanel = ({ onClose, onRouteSubmit, onPublish, routeStats, initialWaypoints, myLocation, alertLocation, center }) => {
  ...
  const geocode = async (query) => {
    try {
      ...
      // Fallback to OpenStreetMap (Nominatim) if no TomTom key
      const res = await axios.get(`https://nominatim.openstreetmap.org/search`, {
        params: { format: 'json', q: query, limit: 1, countrycodes: 'id' }
      });
  ...
  return (
    <div className={`routing-sidebar-container ${isCollapsed ? 'collapsed' : ''}`}>
      <div className="routing-panel-premium glass-panel">
        {/* Header */}
        ...
        <div style={{ marginTop: '0.5rem' }}>
          <button
            ...
            onClick={() => {
              ...
              // Automatically trigger route calculation from my location to the selected point
              onRouteSubmit([start, target], travelMode);
            }}
          >
            <Navigation size={15} /> Navigasi ke Lokasi Ini
          </button>
        </div>
      ...
      {/* Toggle Handle */}
      <button
        className="routing-sidebar-handle"
        onClick={() => setIsCollapsed(!isCollapsed)}
        title={isCollapsed ? "Expand Routing Panel" : "Collapse Routing Panel"}
      >
